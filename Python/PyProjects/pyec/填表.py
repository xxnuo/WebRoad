...
class Selenium填表:
    ...
    @异常检测
    def 打开谷歌(self,驱动路径='.\pyec\chromedriver.exe',无痕模式=True,隐藏窗口=False,隐藏自动化提示=True,隐藏滚动条=False,禁止加载图片=False,禁用插件=False,禁止Javascript=False,禁用JAVA=False,禁止密码保存提示=True,禁用弹窗=False,禁止策略化=False,初始运行=False,沙盒运行=True,协议头='mozilla/5.0 (windows nt 10.0; win64; x64) applewebkit/537.36 (khtml, like gecko) chrome/85.0.4183.83 safari/537.36',代理='',编码='lang=zh_CN.UTF-8',规避BUG=False,谷歌浏览器路径=''):
